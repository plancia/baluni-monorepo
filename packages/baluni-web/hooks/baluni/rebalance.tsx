...
import { batchSwap } from 'baluni-core';
import { BuildSwapOdosParams, TConfigReturn, Tswap } from 'baluni-core/types';
import { fetchPrices, getTokenBalance, getTokenMetadata, getTokenValue } from 'baluni-core/utils';
import { DexWallet, waitForTx } from 'baluni-core/utils/web3';
// import * as api from 'baluni-core/api';
...
import { Builder } from '../../../../packages/baluni-core/src/api/classes/builder';
import { SwapTokenLogic } from '../../../../packages/baluni-core/src/api/logics/odos/';
...
export async function rebalancePortfolioOdos(
  ...
  try {
    if (
      ...
      const logicsData = await swap_logics.build(
        builder.sender,
      ...
      const builderData = await builder.buildTransaction(
        logicsData.approvals,
        logicsData.calldatas,
        logicsData.tokens_return
      );
      console.log('📡 Builder Data:', builderData);

      for (const tx of builderData.txs) {
        const simulate = await dexWallet.wallet.call(tx);
        if (!simulate) {
          console.log('📡 Simulation failed');
          notification.error('Simulation failed');
          return;
        }
        const txExec = await dexWallet.wallet.sendTransaction(tx);
        ...
        console.log(`📡 Tx broadcasted:: ${broadcaster}`);
        if (broadcaster) {
          notification.success('Rebalance successful');
        }
      }
    }
  } catch (e) {
  ...
  try {
    if (
      ...
      const params: BuildSwapOdosParams = {
        ...
        inputTokens: quoteRequestBody.inputTokens,
        outputTokens: quoteRequestBody.outputTokens as {
          tokenAddress: string;
          proportion: number;
        }[],
        slippageLimitPercent: Number(quoteRequestBody.slippageLimitPercent),
