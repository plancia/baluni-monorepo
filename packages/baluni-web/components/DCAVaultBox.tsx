...
import { BaluniV1DCAVaultRegistry } from 'baluni-contracts/typechain-types/contracts/registry/BaluniV1DCAVaultRegistry';
...
const DCAVaultBox = () => {
  ...
  const fetchAllData = async () => {
    ...
    try {
      const [valuationResponse, unitPriceResponse, statisticsResponse] =
        await Promise.all([
          fetch(valuationUrl),
          fetch(unitPriceUrl),
          fetch(statisticsUrl),
  ...
  const getVaults = async () => {
    ...
    const factory = new ethers.Contract(
      vaultRegistry,
      dcaVaultRegistryAbi.abi,
      clientToSigner(signer)
    ) as any as BaluniV1DCAVaultRegistry;
    ...
    for (const vaultAddress of vaultsAddress) {
      ...
      tlvs[vaultAddress] = ethers.utils.formatUnits(
        await vault.totalValuation(),
        baseDecimal
      );
      ...
      const poolERC20 = new ethers.Contract(
        vaultAddress,
        erc20Abi,
        clientToSigner(signer)
      );
      ...
      const quoteAssetContract = new ethers.Contract(
        quoteAsset,
        erc20Abi,
        clientToSigner(signer)
      );
  ...
  const handleVaultClick = (vaultAddress: string) => {
    ...
    const filteredValuationData = allValuationData.filter(
      (data) => data.address === vaultAddress
    );
    const filteredUnitPriceData = allUnitPriceData.filter(
      (data) => data.address === vaultAddress
    );
  ...
  return (
    <div className="container mx-auto p-6 mb-8">
      ...
      >
        <img
          src="https://www.svgrepo.com/download/470882/refresh.svg"
          alt=""
            ...
            {vaults.map((vault, index) => {
              ...
              return (
                ...
                >
                  ...
                  <td>
                    ${Number(vaultData[vault]?.unitPrice).toFixed(5) || '0'}
                  </td>
                  <td>
                    {stats ? (
                      <>
                        <div>
                          ...
                          {stats.unitPrice.daily
                            ? stats.unitPrice.daily.toFixed(2)
                            : 0}
                          %
                        </div>
                        <div>
                          ...
                          {stats.valuation.daily
                            ? stats.valuation.daily.toFixed(2)
                            : 0}
                          %
                        </div>
                      </>
        ...
        <div className="modal modal-open">
          <div className="modal-box w-11/12 max-w-5xl md:w-9/12 bg-base-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-4 text-center">
              <div className="stat">
                ...
                <div className="stat-value">
                  ${Number(vaultData[selectedVault]?.unitPrice).toFixed(3)}
                </div>
              </div>
              <div className="stat">
                <div className="stat-title">TLV</div>
                <div className="stat-value">
                  ${Number(tlvs[selectedVault]).toFixed(4)}
                </div>
              </div>
              <div className="stat">
                <div className="stat-title">Your Liquidity</div>
                <div className="stat-value">
                  {Number(liquidityBalances[selectedVault]).toFixed(5) || '0'}
                </div>
              </div>
              <div className="stat">
                ...
                <div className="flex flex-wrap justify-center text-base items-left">
                  <div className="stat-value">
                    <div className="flex items-center mb-2">
                      ...
                      />
                      <span>
                        {Number(vaultData[selectedVault]?.baseBalance).toFixed(
                          3
                        )}
                      </span>
                    </div>
                    <div className="flex items-center">
                      ...
                      />
                      <span>
                        {Number(vaultData[selectedVault]?.quoteBalance).toFixed(
                          8
                        )}
                      </span>
                    </div>
            ...
            <div className="card  p-4 lg:p-6 w-full">
              <h2 className="card-title text-2xl sm:text-3xl mb-4 sm:mb-8">
                Charts
              </h2>
            ...
            <div className="p-4 rounded shadow mt-4">
              ...
              <div className="text-lg font-semibold">
                {getVaultDescription()?.objective}
              </div>
              <span className="text-lg font-bold mt-2">
                Strategy Description
              </span>
              <div className="mt-2 text-base">
                {getVaultDescription()?.description}
              </div>
            </div>
