...
import baluniPoolAbi from 'baluni-contracts/artifacts/contracts/pools/BaluniV1Pool.sol/BaluniV1Pool';
import poolPeripheryAbi from 'baluni-contracts/artifacts/contracts/pools/BaluniV1PoolPeriphery.sol/BaluniV1PoolPeriphery';
import poolRegistryAbi from 'baluni-contracts/artifacts/contracts/registry/BaluniV1PoolRegistry.sol/BaluniV1PoolRegistry';
...
const SwapBox = () => {
  ...
  const poolsQuery = useReadContract({
    address: poolFactory as `0x${string}`,
  ...
  const assetsFunction = useCallback(async () => {
    const poolAddresses = poolsQuery.data ?? [];
    const assetsPromise = poolAddresses.map((poolAddress: `0x${string}`) => {
    ...
    const assetsResults = await Promise.all(assetsPromise);

    const tokensPromises = assetsResults
      .map((assetResult) => {
        return assetResult.map((asset) => {
    ...
    const symbols = await Promise.all(tokensPromises);
  ...
  const filteredTokensQuery = useQuery({
    queryKey: ['assets'],
    queryFn: assetsFunction,
    enabled: !!signer && !!poolFactory && !!poolPeriphery,
  });

  const filteredTokens = filteredTokensQuery.data ?? [];
  ...
  const previewSwap = async (
    ...
    const amountOut = await periphery.getAmountOut(
      fromToken,
      toToken,
      ethers.utils.parseUnits(amount, decimalsFrom)
    );
  ...
  if (filteredTokensQuery.isLoading) {
    return <Spinner />;
  }
