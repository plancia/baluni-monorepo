...
import * as quoterAbi from '../../abis/uniswap/Quoter.json';
import * as factoryAbi from '../../abis/uniswap/Factory.json';
import * as poolAbi from '../../abis/uniswap/Pool.json';
import {
  AlphaRouter,
  CurrencyAmount,
  SwapType,
...
import { parseToken } from './module.parse-token';
...
async function getPoolData(
  ...
  minRequiredLiquidity: BigNumber
) {
...
async function quoteSwapAmountOut(
  ...
  if (!swapPossible) {
    console.log(
      'La swap non è possibile, evitando la chiamata che fallirebbe.'
    );
...
export async function getBestQuote(
  ...
  const _protocol = (PROTOCOLS as any)[chainId][protocol];
  ...
  const adjAmount = getAdjAmount(
    Number(amount).toPrecision(18),
    tokenADecimals
  );
