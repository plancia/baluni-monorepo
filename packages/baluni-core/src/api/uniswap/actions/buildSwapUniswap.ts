...
import * as factoryAbi from 'baluni-contracts/artifacts/contracts/orchestators/BaluniV1AgentFactory.sol/BaluniV1AgentFactory.json';
...
export async function buildSwapUniswap(
  ...
  const registry = new Contract(
    (deployedContracts as any)[swaps[0].chainId].BaluniV1Registry,
    registryAbi.abi,
    wallet
  );
  ...
  const ApprovalsAgent = [] as any[];
  const Calldatas = [] as any[];
  let TokensReturn = [] as any[];
  ...
  for (const swap of swaps) {
    ...
    if (transferFromSenderToAgent) {
      if (debug)
        console.log('::API::UNISWAP::BUILD_TRANSFER_FROM_SENDER_AGENT');
